...
\date{\textbf{Canadian Student Health Research Forum}\\[0.5em]\scriptsize
  2020 August 25}
\titlegraphic{\flushright\vspace{-0.7\textheight}\input{tikz/turnover}}
\begin{document}
% ------------------------------------------------------------------------------
\begin{frame}
  \maketitle
...
\begin{frame}{Background}
  \pause
  \visible<+->{
  \paragraph{Sexually Transmitted Infections}
  \begin{itemize}
  ...
  \begin{itemize}
    \item from epidemic simulation models
...
\begin{frame}{Key Modelling Concepts}
  ...
  \begin{columns}[t]
    \begin{column}<+->{0.5\linewidth}
      ...
      \visible<+->{\xpar\centering\input{tikz/turnover}}
    \end{column}
    \begin{column}{0.5\linewidth}
      \visible<+->{\paragraph{Assortative Mixing:} like-with-like (vs random) partnerships}
      \visible<+->{\xpar\centering\input{tikz/asso}}
    \end{column}
  \end{columns}
  \tikzpausefalse
\end{frame}
...
\begin{frame}{Research Questions}
  ...
  \xpar
  \begin{enumerate}
    \item equilibrium STI \textbc{prevalence}
    \xpar
...
\begin{frame}{Random mix --- turnover ``homogenizes'' STI prevalence}
  ...
  \begin{columns}
    ...
    \begin{column}{0.2\linewidth}
      \centering\visible<2->{No Turnover (\textbc{A})\xpar\turnoverfalse\input{tikz/toy}}
    \end{column}
    \begin{column}{0.2\linewidth}
      \centering\visible<3->{Turnover (\textbc{B})\xpar\turnovertrue\input{tikz/toy}}
    \end{column}
  \end{columns}
\end{frame}
% ------------------------------------------------------------------------------
\begin{frame}{Random mix --- infer larger risk ratio with turnover $\rightarrow$ higher tPAF+}
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.2\linewidth}
      \centering\visible<1->{No Turnover (\textbc{A})\xpar\turnoverfalse\input{tikz/toy}}
    \end{column}
    \begin{column}{0.2\linewidth}
      \centering\visible<1->{Turnover (\textbc{B})\xpar\turnovertrue\input{tikz/toy}}
    \end{column}
  \end{columns}
\end{frame}
% ------------------------------------------------------------------------------
...
\begin{frame}{Implications}
  ...
  \begin{enumerate}
    ...
    \xpar
    \visible<+->{\item If turnover is \textbf{ignored}: we \textbc{underestimate} impact of reaching \textbf{high risk} groups}
    \xpar
  \end{enumerate}
  \xpar
  \visible<+->{Also relevant to non-STI epidemics \dots}\visible<+->{like \textbf{\textsc{covid}-19}}
\end{frame}
% ------------------------------------------------------------------------------
