...
were central to epidemic persistence in both models.			%SM: not scenarios (be careful as scenarios = counterfactuals from the same model). Here we have two different models or two different modelled systems. Also, it is an increasing tPAF over time-horizons that is linked to R0/Reffective/epidemiologic drivers, not whether or not it get close to a certain value. 
% SB: Crucial.
