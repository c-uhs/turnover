...
$\input{\datapath/values/turnover-prevalence-ratio-high-low.txt}$ from			%SM: and refer to Table 2 here as well as in section below
the model with turnover (($\delta_H = 5$)
~vs~%
$\input{\datapath/values/no-turnover-prevalence-ratio-high-low.txt}$ 				%SM: and refer to Table 2 here as well as in section below
...
\label{ss:res-infer}
%SM: moved/edited the text re: predicted prevalance ratios to the above section as I think fits better there right after talking about shrinking prevalence ratios with turnover. In current section, best to go right into fitted results. 
...
in the high, medium, low risk groups, and total population, respectively			%SM: suggest using term respectively when listing multiple items? also double check that there is symmetry (e.g. with and without turnover --> then the numbers that follow should reflect the same order).
...
estimated by the fitted model with turnover and 				%SM: check consistency re: estimated, projected, predicted. when referring to the tPAF. I think various terms used in paper, and all are fine so suggest choosing one for consistency.
...
indicating that unmet prevention and treatment				%SM: why just treatment? better to allow for prevention and treatment needs?
needs of the high risk group
were central to epidemic persistence in both models.			%SM: not scenarios (be careful as scenarios = counterfactuals from the same model). Here we have two different models or two different modelled systems. Also, it is an increasing tPAF over time-horizons that is linked to R0/Reffective/epidemiologic drivers, not just whether or not it get close to a certain value although that is correct re: approaching 1 --> tPAF approaching 0.4 over 50  years could also be central to epidemic persistence. Given that we are not actually simulating a real STI --> 50 years does not "mean" something here, and so would couch this sentence more carefully.
% SB: Crucial.
The model with turnover estimated a larger tPAF at all
time-horizons compared with the tPAF estimated by the model
without turnover.
%SM: said this in discussion so not sure need to also say in results. removed to make the results a bit more succinct and crisp i hope.
...
The larger tPAF via the model with turnover
stemmed from more risk heterogeneity (Table 2) which
led to more onward transmission from the unmet 
