...
% SM: actually I think it helps to avoid confusion when I read it now. So good suggestion by LW! :)
% JK: Yes, good suggestion @LW! I just mean the middle group is hard to explain,
%     so still they are missing from the text, but included in the figures.
...
controlled by duration of time spent in the highest risk group.
...
% and the turnover-prevalence relationship transitioned from positive to negative,
% JK: ^ I think this can be omitted?
...
\begin{minipage}[t]{0.475\linewidth}
  ...
  \caption{Relationship between equilibrium STI prevalence
    ...
    Regions A~and~B denote where prevalence increases and decreases
    with turnover, respectively.
...
added more incident infections to the high risk group via incidence
than were lost via turnover
(Figure~\ref{fig:dX-I-high}),
which explains why equilibrium prevalence in the high risk group
...
in the high risk group under fast versus slow rates of turnover.
...
decreased with turnover, incidence also decreased
...
with infectious and treated individuals.
...
Incident infections also increased
with slow versus no turnover in the low risk group
...
\par
Faster turnover still contributed a net gain of infectious individuals
...
Since these two tendencies act to bring prevalence between the two groups
closer together, we summarize these two tendencies as
...
This reflects the homogenizing effect of turnover on group-specific prevalence.
