...
% ==================================================================================================
\subsection{Influence of magnitude of turnover}\label{ss:exp-turnover}  %% frame the duration of infectiousness part as a sensitivity analysis
...
to the inverse of how long individuals remain in the highest risk group. %start by something like this to orient the reader and help with interpretation before results section
That is, a shorter period of time spent engaged in highest risk means a higher magnitude of turnover.
We first consider the influence of turnover at a fixed duration of infectiousness.   % more active voice in general
...
the duration of individuals in the high risk group $\delta_H$.  %when reading 'following way' - anticipated bullet points or a ":", etc. so suggest removing that bit
Under the condition that the distribution of model entrants $\bm{\hat{e}}$ remains equal  %see comment before about 'model entrants'
...
whose elements all scale inversely with the duration in the high risk group $\delta_H$.       %very nice. in the discussion - point out that duration in the highest risk group is the most common empiric estimate we would have available, vis a vis sex work for example and cite Watts et al
...
\par
% --------------------------------------------------------------------------------------------------
\paragraph{Experiment 2.2: Influence of the magnitude of turnover at various treatment rates} %suggest reframing this as a sensitivity analyses so the primary aims are about turnover
...
The duration of time spent in the high risk group $\delta_H$ is varied from 3~to~33 years as with Experiment 2.1. %avoid 'as before'; instead reference/bookmark the exact section
...
% ==================================================================================================
\subsection{Influence of turnover on the contribution of highest risk group to overall transmission }\label{ss:exp-turnover-fit}
Next we examine the influence of turnover when fitting models to observed data
on disease prevalence across risk groups. We use two models: one with turnover (%refer to Figure to show which one)
and one without turnover (%refer to Figure to show which one).
Both models include population growth and 3 risk-groups.
We then fit the following parameter in both models: relative contact rates between the highest and lowest risk group. %or what was varied/fit
That is, we fit the models to equilibrium disease prevalence in the highest and and lowest risk groups  %does this get us the same overall (total population) prevalence? 
and infer the degree of risk heterogeneity in the population. We then estimate the contribution of the highest risk group to overall transmission from both models.  %degree used below
...
%% move this to discussion --> "TPAF estimates the proportion of cumulative new infections which are attributable to
...
%the part about comparing non-fitted scenarios does not come through here or in Experiment 3.2. I wonder if that diagram we drew a long time ago on a whiteboard re: the comparisons could be helpful? 
The Base model (turnover) and model variant V3 (no~turnover) are calibrated to
25\% prevalence in the high risk group, and
5\% prevalence in the low risk group at equilibrium.  %they should also reproduce overall prevalence too? what do you think?
...
%% Suggest we put hypotheses in introduction
...
% before and after fitting to group-specific prevalence, as described above.  --- before and after suggests priors and posteriors in the context of model fitting