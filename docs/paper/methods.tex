...
\end{figure}
\par
To address our first objective, we first
compared the equilibrium prevalence (in each risk group and in the
...
We then explored the mechanisms that explain the observed relationships by.... %SM: ??doing what? 
% --------------------------------------------------------------------------------------------------
...
To address our second objective, we compared the fitted (posterior) ratio of partner numbers $C_H~/~C_L$		%in introduction, helpful if objectives listed.
...
counterfactual where X is turned off starting at the fitted equlibrium prevalence. 	%SM: the counterfactual is not defined clearly enough. 'assuming no transmission from a risk group' is not precise enough. what exactly did we do? set beta to zero within the risk group? is it a bi-direcitonal TPAF or unidirectional? Remember my talk at CROI where I had to clearly say what the counterfactual was.
That is, in the counterfactual scenario, individuals in the high risk group could not become 
infected nor could those already infectious transmit to another person.		%SM: correct? or edit for accuracy but needs to be clearly articulated 

The TPAF was calculated over different time-horizons (1 to 30 years) as:		%SM: give the time-horizon shown in results. helps the reader know what is coming.
\begin{equation}
\textsc{tpaf}_i(t) = \frac{I_0(t) - I_i(t)}{I_0(t)}							%SM: on my first look i thought you were just generating a relative risk from this equation using instatnatnous number of infectious. Should be integrated over I from time-equlibrium to time-outcome. the words should not be needed to explain the equation (e.g. integral would already tell us = cumulative, and the from/to would be clear re: that we need a time-horizon for calculating. also i and -0 are not clear as the base-case and counterfactual - suggest using terms like b vs. c (b= base-case; c=counterfactual). See how we have written the equation before to make it clear and can cite: PMID: 24056777
\end{equation}
...
Both $I_0(t)$ and $I_i(t)$ were calculated					%SM: I do not understand the sentence. Please revise for clarity.
