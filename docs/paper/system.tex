...
% --------------------------------------------------------------------------------------------------
\paragraph{Constraint 1: Constant Group Size}			%number the example constraints. Similar to Box 1 --> a box or flow diagram to show the various approaches to constraints will be useful - e.g. either constant group size or constant turnover rates....or as subsets of the third element in Box 1 (under Turnover) --> leads nicely into decisions based on data and assumptions. Can discuss at next PM if not clear from my written comments.
