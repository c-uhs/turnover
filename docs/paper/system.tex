...
% JK: (4): done, but I had trouble finding one for (3)
...
%     but I can't really find a citation for the accuracy of
%     exponential piecewise approximations of variable rate exponential growth,
%     since most people do linear piecewise approximations of fixed rate exponential growth.
\par
...
% --------------------------------------------------------------------------------------------------
\paragraph{Constraint Type 1: Constant group size}
...
% TODO: discuss assumption of "constant" sex work population size over time?
...
% JK: not sure if there is additional normalization needed, actually
% SM: explain this to me?
% JK: nevermind - I was concerned that dividing the derivative by N is somehow improper,
%     but derivatives are linear operators, so all good :)
...
% SM: what are these constraints? the specified elements?
%     its not clear until after reading the next section
%     suggest introducing/listing things before moving to subsequent paragraph/section.
% JK: hopefully with the new preamble at the end of \ref{sss:params-turnover}
%     (just before Constraint Type 1), this should be clear now? Let me know if not...
% --------------------------------------------------------------------------------------------------
...
% JK: given the clarification in text (below), is this footnote also necessary?
...
% SM: redundant? or is it because it sums to 1 and therefore is a 'known' value?...
%     not sure redundant is the correct term?
% JK: you're right, I wasn't sure how to describe it.
%     I changed to "be either redundant or violate the sum-to-one rule" - is that better?
...
% SM: am not sure I follow why it is not necessary to supply the summation constraint?
%     is it a check?
% JK: hopefully the new Appendix~\ref{aa:eqs-e-redundant} clarifies!
% SM: similar re: sentence above? specify which sections 'below'
%     but only mention redundancies in sentence above, where are the inconsitencies?
%     rephrase sentence for clarity and precision
% JK: I think easiest to just take this reference to "other redundancies and inconsistencies"
%     and introduce redundancies and inconsistencies as they come up in Constraints 3 and 4.
% --------------------------------------------------------------------------------------------------
\paragraph{Constraint Type 3: Group duration}
\label{con:group-dur}
% SM: can discuss at next PM, but a brief table of what type of data and assumptions
%     for each constraint would be useful for epidemiology audience
%     (thinking about readers like Sheree, etc.)
...
% SM: for each paper/model --> define their objectives/constraints for turnover,
%     and then talk about how they went about;
%     then link to your unified approach explicitly and clearly.
% --------------------------------------------------------------------------------------------------
\par
...
% --------------------------------------------------------------------------------------------------
\par
\citet{Boily2015} simulated turnover among $G = 7$ risk groups, -- \frame{TODO}.
\par
